<script lang="ts">
    ...
    let{parentId,fetchComments,childComment,editMode = $bindable()} : {parentId:number,fetchComments:()=>void,childComment:App.AnonCommentDto,editMode:boolean} = $props();
...
<div class="childComment" style="padding:10px; margin-left: 20px;">
    ...
    <div style="display: flex; padding-top: 10px; align-items: center;">
        {#if editMode}
            <AnonCommentEdit {fetchComments} comment = {childComment} bind:editMode></AnonCommentEdit>
        {:else}
